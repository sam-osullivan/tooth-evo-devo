...
module coreop2d   !WARNING: optimized version in order not to add

!but the margins are as coreop2d can not grow sideways after certain value radibi
...
real*8, public, allocatable  :: marge(:,:,:)  ! internodes positions for each cell x,y,z
      ...
      do kk=2,ncz-1
        ...
        do j=1,nvmax
          if (vei(i,j)/=0) then 
            ii=vei(i,j)
            if (ii==ncals) then
!              if (k/=1) then     !ACHTUNG This is because the margins of the tooth have much activator: Ames estimated inefficient
...
subroutine biaixbl
  do i=1,ncils-1
    if (malla(i,2)<-tadi) then
        if (q3d(i,1,1)<bil) then  ! Won't reset activator concentration to value of bias; to make Ina work in borders.
            q3d(i,1,1)=bil
        end if
    else
      if (malla(i,2)>tadi) then
        if (q3d(i,1,1)<bib) then  ! Same as above.
            q3d(i,1,1)=bib
        end if
      end if
    end if
...
module esclec
use coreop2d
public:: guardaforma,guardaveins,guardapara,llegirforma,llegirveins,llegirpara
...
integer imap
  ...
  parap(20+ng+ngg,imap)=radibi ; parap(14+ng+1,imap)=tadif  ;print*,"tadif",tadif,14+ng+1
  parap(15+ng+1,imap)=fac ; parap(21+ng+ngg,imap)=radibii ; parap(12,imap)=ncals
  parap(22+ng+ngg,imap) = ina ;print*,"ina",ina,22+ng+ngg
  parap(23+ng+ngg,imap) = umgr ;print*,"umgr",umgr,23+ng+ngg
...
character*6 dcq
character*31 nff
character*51 nf,nfoff,nfes,accaufolder,caufolder
character*4 chq
character*6 chs
character*8 chv
character*2 di
...
call getarg(3,cau) !fitxer sortida  !output file
call getarg(4,cad) !iteracions      ! number iterations
call getarg(5,cass) !numero de steps  !repetion of the iterations, it safe a file at this step, so an example input of fileInput fileOutpt 1000 10 will run from 1000x10 iterations, saving an putput file every 1000 iterations
